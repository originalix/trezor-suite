...
import { UsbTransport } from '../src/transports/usb';
...
describe('Usb', () => {
    ...
    describe('with initiated transport', () => {
        ...
        it('stop aborts pending promises', async () => {
            await transport.enumerate();
            const acquirePromise = transport.acquire({ input: { path: '123', previous: null } });
            transport.stop();
            abortController.abort();
            expect(acquirePromise).resolves.toEqual({
                success: true,
                error: 'todo: this does not work',
            });
        });
    });
});
